...
int main ( int argc, char *argv[] ) {
   ...
   int endDate[11]= {dd2Str[3], mm2Str[3],yyyy2Str[5]};
    ...
    int startdate = DayofYear[mm-1] + dd -1  ;
    int enddate = DayofYear[mm2-1] + dd2 ;


    int count = 0,i;
    for (i = atoi(yyyyStr); i < atoi(yyyy2Str);i++)
    {
    if(i%4==0 && i%400==0) 
    count+= 366;
   ...
   if ( argc < 3 ) {
      printf ( "Usage: ./CalculatorE dd-mm-yyyy dd2-mm2-yyyy2 \n" );
      exit ( 1 );
   }
    ...
    int tempyear = atoi(yyyyStr);

         int j = 0;
       while (tempyear < yyyy2){
     if (tempyear%400 == 0 || tempyear%4 == 0 && tempyear%10!=0){
       j++ ;
      }
     tempyear++;
}

    int numberofdays = (enddate - startdate) + j ;

      if (yyyy > yyyy2) {
     printf("Error - The first year cannot be greater than the second year" );
     }
     else if (yyyy2 > yyyy) {
     (numberofdays = (yyyy2 - yyyy)* 365 + (enddate - startdate) );
      } 
